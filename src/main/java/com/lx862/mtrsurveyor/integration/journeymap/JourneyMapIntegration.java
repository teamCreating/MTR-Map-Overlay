...
public final class JourneyMapIntegration {
    ...
    /**
     * Called every client tick (from MTRSurveyor). Tracks dimension changes
    ...
    public static void onClientTick() {
        ...
        if (!isJourneyMapLoaded()) {
            if (!journeyMapMissingLogged) {
                ...
                MTRSurveyor.LOGGER.info("[MTRSurveyor] JourneyMap not installed - landmark integration disabled");
            }
            needsSync = false;
            return;
        }
        ...
        try {
            ...
            if (clientData == null) {
                MTRSurveyor.LOGGER.debug("[MTRSurveyor] MTR client data not available yet, will retry...");
                return;
            }
            ...
            MTRSurveyor.LOGGER.warn("[MTRSurveyor] JourneyMap classes not available: {}", e.getMessage());
            needsSync = false;
        } catch (Throwable e) {
            MTRSurveyor.LOGGER.error("[MTRSurveyor] Error during landmark sync tick", e);
            needsSync = false;
        }
    }
     ...
     * {@code /mtrsurveyor testMarker}) to verify the JourneyMap integration
     * works end to end.
     *
    ...
    public static String placeTestMarker() {
        ...
        try {
            ...
            MTRSurveyor.LOGGER.error("[MTRSurveyor] Failed to place test marker", e);
