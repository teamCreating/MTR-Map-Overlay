...
| JourneyMap | 仅在全屏地图显示车站、站台和车辆段标记。 |
...
4. 打开 Xaero's World Map 或 JourneyMap 全屏地图。Xaero 左上角的 `ROUTES`、`TRACKS` 按钮可切换图层；悬停在线路或图标上可查看详情。
...
| `/mtrmap config routeLines <true\|false>` | 显示或隐藏 Xaero 路线色带。 |
| `/mtrmap config trackLines <true\|false>` | 显示或隐藏 Xaero 轨道层。 |
...
| [`integration/journeymap/`](src/main/java/com/lx862/mtrmap/integration/journeymap) | 可选的 JourneyMap v2 插件及全屏 `MarkerOverlay` 生命周期。 |
...
数据流：MTR 模拟器或客户端数据 → 按维度管理的 `MapDataCache` → Xaero 绘制器或 JourneyMap 标记。有服务端组件时，客户端先探测各维度内容哈希，只请求变化的快照，再重组分块数据并更新缓存；否则使用 MTR 客户端半径范围内的数据。
