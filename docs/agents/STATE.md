...
- **v1.4.7**（新增 Fabric 1.21.1 并行构建，保留 NeoForge）
...
- NeoForge 与 Fabric 使用各自入口、配置、客户端命令及网络适配，共用地图数据、渲染与协议 v5。v1.4.7 双平台 JAR 已作 GitHub 预发布；两平台构建通过，NeoForge 测试通过，待 Ben 实机验证。
